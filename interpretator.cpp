...
void translete(std::string& code_item);
void memory();
...
void creative(std::vector <std::string>& tokens)
{
	...
	for (int i = 0, turn = 0; i < tokens.size(); i++, turn++)
	{
		...
		else if (tokens[0] == "tox") {
			std::string tmp = tokens[i];
			var.index = s_v.size();
			s_v.push_back(tmp);
		}
		else if (tokens[0] == "erkuamb") {
			double tmp = std::stod(tokens[i]);
			var.index = d_v.size();
			d_v.push_back(tmp);
		}
		else if (tokens[0] == "erk") {
			bool tmp;
...
void condition(std::vector <std::string>& tokens)
{
	...
	for (int tmp = 0; tmp < tokens.size(); tmp++)
	{
		if (tokens[tmp] == "Shirakstyle" || tokens[tmp] == "menc" || tokens[tmp] == "poqr" || tokens[tmp] == "poqhav" || tokens[tmp] == "hav" || tokens[tmp] == "nos")
		{
...
void translete(std::string& code_item)
{
	...
	if (tokens[0] == "amb" || tokens[0] == "erkuamb" || tokens[0] == "erkar" || tokens[0] == "tox")
	{
		unboxing(tokens);
		operation(tokens);
		creative(tokens);
	...
	else if (tokens[0] == "Kamel")
	{
		int begin = indexx, finish = indexx+1, count = 0;
		bool tmp = 1;
		for (; tmp ; finish++)
		{
			if (code[finish] == "start") count++;
			else if (code[finish] == "end") count--;
			if (count == 0) tmp = 0;
		}
		finish--;
		indexx++;
